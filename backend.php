...
//generates the welcome screen 
	...
	//2 reasons for the "10 minutes" thing: -exchange rate volatility; -wallet access through the internet

		$response = array(
						'payment_status' => 'welcome_screen',
						'html_body' => "
										<h2>Welcome to the Bitcoin WiFi hotspot!</h2>
										This is a paid hotspot.
...
//$_SERVER['remote_addr']; //get IP of client; only way to associate a BROWSER with a filtered MAC/IP system :)
...
//asks Bitcoind for a fresh destination address, and saves it in the database, associated with the IP of the client.
	...
	//!!!! we need to verify whether the IP is already in the database. if so, that means the client is already connected

	//create new instance
	$bitcoin = new Bitcoin('hotsp0t','brabrabra','192.168.56.101','8332'); //host and port are optional if localhost and default port
...
function display_payment_status(){
